...
// Let's remove the "Remove Black BG" block:
code = code.replace(/\{overlayImage && \(\n\s*<label className="flex items-center gap-2 cursor-pointer group px-1">[\s\S]*?<\/label>\n\s*\)\}/, "");

// Let's remove Transparent BG Toggle entirely. We need to be careful with regex here.
// Look for Transparent BG Toggle comment
